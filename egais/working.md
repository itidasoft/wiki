...
2.  Полученный документ Инвентаризация ЕГАИС заполните марками. Либо расчетными остатками из Айтиды, либо, запросив марки из ЕГАИС. Если не требуется проведения общей инвентаризации марок ЕГАИС, то предварительно из документа можно удалить лишние строки, оставив только те, по которым необходимо сверить остатки разливной продукции.
3.  После заполнения документа марками в списке действий (кнопка Действия) выберите вариант **Создать из отмеченных строк инвентаризацию для разливной продукции (ОБЩЕПИТ).** Если нужно создать новый документ не для всех позиций исходного документа, то отметьте в списке только необходимые.
...
5.  Новый документ будет подробно развернут в разрезе справок Б и марок, что упростит заполнение фактического количества.
...
7.  Дальнейшие действия аналогичны инвентаризации регистра марок ЕГАИС. В кнопке **Действия** необходимо выбрать "**Запросить из ЕГАИС остатки марок**"
    ...
    В ответ на действие, очередь запросов в ЕГАИС будет заполнена необходимыми запросами. После отправки всех запросов и получения ответов, колонка **Количество в ЕГАИС** будет заполнено полученными из ЕГАИС данными.
