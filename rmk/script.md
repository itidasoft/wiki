...
ELSE
	ОТПРАВИТЬСООБЩЕНИЕ( _ДЕСКРИПТОРОКНА, _СООБЩЕНИЕУСТАНОВИТЬЗНАЧЕНИЕПОЛЯ, "echeque", false );
RETURN true;
```
...
# Настройка запрета продажи алкогольной продукции
Стандартное разрешенное время для продажи алкогольной продукции настраивается в схеме РМК на вкладке "ЕГАИС и скрипты". Данная настройка проверяет только указанное разрешенное время продажи без контроля праздничных дней с запретом продажи алкоголя.
Начиная с версии 4.7.1 добавлена возможность указания скрипта, проверяющего разрешенное время для продажи алкоголя

![alcotime-script.png](/images/rmk/ext/alcotime-script.png){.align-center}

В рамках скрипта доступно гибкое определение условий по разрешенному времени продажи на встроенном языке скриптов.
В качестве примера готового скрипта запрета реализован скрипт проверки для Вологодской области, где разрешенное время в будни установлено в промежутке с 12:00 до 14:00 и в выходные с 8:00 до 23:00. Так же присутствуют общероссийские и региональные праздники с полным запретом продажи алкоголя. 
Для активации готового скрипта достаточно в скрипт проверки добавить следующий текст:

```js
RETURN Сервис.СкриптПроверкиОграниченияПродажиАлкоголя();
```