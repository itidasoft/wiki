...
Основная единица измерения должна быть единице измерения объема, например литр. Для алкогольной продукции, учитываемой в ЕГАИС как разливная продукция в поле Объем необходимо указать значение 1 литр. В поле GTIN необходимо задать верный GTIN продукции.
...
Схема работы с маркированными разливными пивными напитками подразумевает необходимость отправки данные в **ГИС МТ**. Поэтому, перед начало работы необходимо произвести настройку модуля **Маркировка** для работы организации с товарной группой **Пиво и пивные напитки**. В случае использования распределенной базы настроенное описание будет передано в кассовые узлы автоматически. Поэтому, в кассовых узлах нет необходимости осуществлять отдельную настройку Предприятий.
...
При нажатии кнопку будет открыта форма управления кранами. В ней необходимо добавить группы кранов и для каждой группы заполнить список кранов. Доступ к управлению группами и кранами имеют сотрудники с правами администратора.
...
2.  Возможная схема работы. Если на рабочем месте кассира есть КЭП для отправки документов в ГИС МТ, то документ может быть отправлен в непосредственно в момент его создания.
...
В момент записи система проверит доступный остаток на кране. Если остатка достаточно, что сформированная тара будет записана и доступна к продаже.
...
В случае необходимости предусмотрена процедура печати этикетки. Этикетка может облегчить продажу на кассе, т. к. обычно содержит ШК продукции и её единицу измерения (объем бутылки). На этикетке не нужно распечатывать код маркировки, т. к. актуальный код маркировки будет определен в момент записи чека продажи этой тары. В перечне актуальных печатных форм уже предусмотрен стандарный шаблон этикетки с наименованием **"Этикетка - маркировка (розлив пива в тару)"** для использования при розливе в фасованную тару.
...
В момент подбора товара в чек и / или изменения количества товара в строке чека производится проверка наличия необходимого объема напитка. В случае успешной проверки, в строке товара будет выведен зеленый маркер. ![Изображение выглядит как текст, снимок экрана, Шрифт, линия Автоматически созданное описание](/images/rmk/draftdrinks/0e954177e1b25ca8bca875a980fa908f.png). Если проверка не пройдена, то будет выведено сообщение о нехватке обхема к продаже, а в строке будет выведен красный маркер ![Изображение выглядит как текст, Шрифт, снимок экрана Автоматически созданное описание](/images/rmk/draftdrinks/8f30858282dedfc4c1eeb16ab3ca1725.png). Данные маркеры являются только информацией для кассира о последней проверке наличия. В момент закрытия чека для всех строк будет произведено определение кода маркировки и анализ возможности списания объема. Поэтому, в случае красного маркера есть возможность до закрытия чека установить кег на кран и добавить необходимый объем к продаже. В случае зеленого маркера, существует вероятность, что к моменту закрытия чека объем будет продан на другой кассе и его не хватит к продаже в текущем чеке.
