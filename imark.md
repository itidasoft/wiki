...
\- **Удобный клиент для управления** и настройки сервера iMark. Варианты взаимодействия с сервером: 1) API; 2) Приложение для Windows; 3) Веб интерфейс (скоро).
...
С процессом подготовки Айтида РМК к разрешительному режиму, а так же с настройкой и установкой модуля iMark можно озникомиться в обучающем видео:  
...
>   Внимание! При установке Айтида iMark в связке с кассовым ПО Frontol xPOS3 версии 3.12.3 рекомендуется **включить** **этот параметр**, т. к. Frontol xPOS3 посылает запрос с неполным КМ на проверку и в случае получения ошибки локальной проверки более ничего не отправляет. Без локальной проверки xPOS получает данные онлайн проверки из ГИС МТ и посылает повторно полный код маркировки на проверку.
{.is-warning}
