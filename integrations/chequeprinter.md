...
4. `"{Alignment=0;}Номер стола " + НОМЕРСТОЛА + ` - выведет указанный номер стола (если подключен [Информер](#подключение-обработчика-для_запроса-произвольных-данных) ). Выравнивание строки будет слева, т.к. применен параметр Alignment со значением 0.

5. `"{Alignment=0;}Количество гостей " + КОЛИЧЕСТВОГОСТЕЙ +` - выведет указанное количество гостей (если подключен [Информер](#подключение-обработчика-для_запроса-произвольных-данных) ). Выравнивание строки будет слева, т.к. применен параметр Alignment со значением 0.
