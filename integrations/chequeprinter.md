...
Пример шаблона для печати строк марок:
```js
...
4. `"{Alignment=0;}Номер стола " + НОМЕРСТОЛА + ` - выведет указанный номер стола (если подключен [Информер](#Подключение_обработчика_для_запроса_произвольных_данных) ). Выравнивание строки будет слева

5. `"{Alignment=0;}Количество гостей " + КОЛИЧЕСТВОГОСТЕЙ +` - выведет указанное количество гостей (если подключен [Информер](#Подключение_обработчика_для_запроса_произвольных_данных) ).
